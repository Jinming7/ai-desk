...
- preview validation serves `support-preview`
- preview-originated new builds write only to `support-preview`
...
Additional shared-DB rule:

- a validated build created in `support-local` may be explicitly promoted into `support-preview` or `support-prod`
- such promotion must use an explicit publication action and alias build linkage, not a fresh rebuild of identical content just to change environment
- the promoted target build must keep provenance to the source build through explicit linkage so rollback, validation lookup, and artifact resolution remain traceable
...
- explicit operator-approved cross-space promotion through the publication path
- this is a pointer-and-alias operation, not implicit serving leakage and not a permission to let runtime read the wrong space by default
...
- `kb_publications` is the canonical serving truth for rollout and rollback decisions
- `kb_serving_versions` may be inspected only as compatibility metadata to detect drift against the active publication, never as an independent runtime serving truth
- schema-first is a hard constraint for rollout and promotion contracts; publication payloads, rollback payloads, diagnostics, and runtime stage contracts must evolve through explicit schema and TypeScript contract changes first, not ad-hoc SQL, regex routing, or environment-specific shortcuts
