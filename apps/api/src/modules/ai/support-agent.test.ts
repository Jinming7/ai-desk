...
test("runSupportSearchAgent supervisor-domain runtime recovers deployment sizing from full deploy docs even when retrieved headings are noisy", async () => {
  const mutableEnv = env as {
    FEATURE_SUPPORT_AGENT_SINGLE_AGENT_RUNTIME?: boolean;
    OPENCLAW_AGENT_ID_SUPPORT_MAIN?: string;
    LOCAL_DOCS_COM_PATH?: string;
  ...
  globalThis.fetch = (async (input: string | URL | { url?: string | URL }) => {
          ...
          <body>
            <main>
              <article>
                ...
                <p>Linux distribution support depends on the documented matrix.</p>
                <h2>Prepare 4 servers</h2>
                <table>
                  <thead>
                    <tr>
                      <th>Users</th>
    ...
    if (requestUrl === "https://docs.ones.com/developer/abilities/extensions/form-control") {
      return new Response(
        `<!doctype html>
        <html lang="en">
          <body>
            <main>
              <article>
                <h1>Form Custom Control</h1>
                <p>Private Deployment SAAS</p>
              </article>
            </main>
          </body>
        </html>`,
    ...
    throw new Error(`unexpected fetch in noisy deployment sizing recovery test: ${requestUrl}`);
  }) as typeof globalThis.fetch;
  ...
  adapter.planSupportDispatch = async (input) => ({
    ...
    caseFrame: {
      goal: "Identify the documented per-node hardware resource requirements for a private/self-hosted deployment.",
      symptom: "User needs sizing guidance for private deployment nodes, specifically CPU, memory, and disk per node.",
      object: "private deployment node infrastructure requirements",
      action_type: "capability_confirmation",
      deployment_model: "private_deployment",
      product_area: "deployment",
      constraints: [
        "Must use published KB/docs only.",
        "Need per-node values, not vague cluster-level guidance.",
        "Need CPU, memory, and disk requirements specifically."
      ],
      missing_critical_info: [],
      retrieval_queries: [
        "private deployment per-node CPU memory disk requirements",
        "self-hosted deployment node sizing CPU RAM storage"
      ],
      question_type: "capability_confirmation",
      specialist_agent: "behavior-specialist",
      answer_contract: "State the documented per-node resource requirements first.",
      ...
      query_plan: {
        concept_queries: [
          "private deployment infrastructure requirements",
          "self-hosted node sizing requirements",
          "minimum recommended hardware requirements deployment"
        ],
        object_queries: [
          "private deployment node CPU requirement",
          "private deployment node memory requirement",
          "private deployment node disk requirement"
        ],
        behavior_queries: [
          "how private deployment requirements vary by node role",
          "whether deployment docs specify minimum vs recommended node resources"
        ]
      },
      required_doc_kinds: ["deployment_runbook", "product_guide", "rules", "troubleshooting"]
    },
    retrievalQueries: [
      "private deployment per-node CPU memory disk requirements",
      "self-hosted deployment node sizing CPU RAM storage"
  ...
  const references: SearchReference[] = [
    {
      documentId: "doc:docker-k3s-migration",
      evidenceId: "chunk:docker-k3s-migration",
      title: "Docker迁移K3S方案",
      snippet: "Migration guidance for Docker-based installations.",
      sourceUrl: "https://docs.ones.com/zh-Hans/deploy/prepare/deployment-requirements",
      path: "deploy-docs/data/docker-to-k3s.cn.md",
      headingPath: "Docker迁移K3S方案",
      authority: "canonical_visible",
      sourceType: "github_kb",
      score: 1.31,
      retrievedAt: "2026-04-14T07:22:08.473Z"
    },
    {
      documentId: "doc:deployment-requirements-xinchuang",
      evidenceId: "chunk:deployment-requirements-xinchuang",
      title: "信创环境要求（可选）",
      snippet: "Private deployment environment guidance.",
      sourceUrl: "https://docs.ones.com/zh-Hans/deploy/prepare/deployment-requirements",
      path: "deploy-docs/prepare/deployment-requirements.md",
      headingPath: "信创环境要求（可选）",
      authority: "canonical_visible",
      sourceType: "github_kb",
      score: 1.468,
      retrievedAt: "2026-04-14T07:22:08.473Z"
    ...
    {
      documentId: "doc:deployment-requirements-os",
      evidenceId: "chunk:deployment-requirements-os",
      title: "操作系统要求",
      snippet: "Linux distribution support depends on the documented matrix.",
      sourceUrl: "https://docs.ones.com/zh-Hans/deploy/prepare/deployment-requirements",
      path: "deploy-docs/prepare/deployment-requirements.md",
      headingPath: "操作系统要求",
      authority: "canonical_visible",
    ...
    {
      documentId: "doc:deployment-requirements-prepare-servers",
      evidenceId: "chunk:deployment-requirements-prepare-servers",
      title: "1.2.1 准备4台服务器",
      snippet: "Prepare the required servers before installation.",
      sourceUrl: "https://docs.ones.com/zh-Hans/deploy/prepare/deployment-requirements",
      path: "deploy-docs/prepare/deployment-requirements.md",
      headingPath: "1.2.1 准备4台服务器",
      authority: "canonical_visible",
      sourceType: "github_kb",
      score: 1.282,
      retrievedAt: "2026-04-14T07:22:08.473Z"
    ...
    {
      documentId: "doc:form-custom-control",
      evidenceId: "chunk:form-custom-control",
      ...
      headingPath: "Form Custom Control",
      authority: "canonical_visible",
      sourceType: "github_kb",
      score: 1.436,
      retrievedAt: "2026-04-14T07:22:08.473Z"
  ...
  try {
    const result = await coreRunSupportSearchAgent({
      ...
      adapter,
      orchestrator: orchestrator as never,
      idempotencyKey: "support-supervisor-domain-deployment-sizing-recovers-from-noisy-headings",
      runtime: {
        deliveryMode: "async_job",
        overallTimeoutMs: 240000,
        requestStartedAtMs: Date.now()
    ...
    assert.equal(result.result.support_answer?.render_variant, "behavior");
    assert.equal(
      result.result.citations.some((item) => item.path === "deploy-docs/prepare/deployment-requirements.md"),
      true
    );
    assert.equal(result.result.citations.some((item) => item.id === "chunk:form-custom-control"), false);
    assert.match(result.result.answer, /50/i);
    assert.match(result.result.answer, /4 cores/i);
    assert.match(result.result.answer, /8 gb/i);
    assert.match(result.result.answer, /200 gb/i);
...
test("runSupportSearchAgent supervisor-domain fallback keeps private deployment password reset on deployment how-to despite oauth api noise", async () => {
  ...
  globalThis.fetch = (async (input) => {
            ...
            <body>
              <main>
                <article>
                  ...
                  <p>Use this recovery procedure when SMTP-based delivery is unavailable.</p>
                  <h2>Procedure</h2>
                  <ol>
                    <li>Log in to the deployment host with operator access.</li>
                    <li>Open the documented recovery flow for the private deployment environment.</li>
                    <li>Reset the administrator password directly on the host-side recovery path.</li>
                    <li>Sign in again and confirm administrator access works.</li>
                  </ol>
                </article>
              </main>
            </body>
    ...
    throw new Error(`unexpected fetch in deployment reset fallback test: ${requestUrl}`);
  }) as typeof fetch;
  ...
  const adapter = createAdapter({}) as OpenClawAdapter & {
    ...
    planSupportCase?: (...args: unknown[]) => Promise<unknown>;
    judgeSupportAnswer?: (...args: unknown[]) => Promise<unknown>;
    writeApiSpecialistAnswer?: (...args: unknown[]) => Promise<unknown>;
    writeDeploymentDomainAnswer?: (
      input: OpenClawSupportSpecialistInput,
      idempotencyKey: string,
      runtime?: OpenClawRuntimeContext
  ...
  adapter.writeApiSpecialistAnswer = async () => {
    throw new Error("api specialist should not run for deployment password reset");
  };
  adapter.writeDeploymentDomainAnswer = async () => {
  ...
  const references: SearchReference[] = [
    {
      documentId: "doc:oauth-openapi-scope",
      evidenceId: "chunk:oauth-openapi-scope",
      title: "GET request methods",
      snippet: "When calling this API, ensure your app's scope declaration includes read:appcenter:user.",
      sourceUrl: "https://docs.ones.com/developer/openapi/auth/scope",
      path: "open-docs/docs/openapi/auth/scope.md",
      headingPath: "GET request methods",
      authority: "canonical_visible",
      sourceType: "github_kb",
      score: 0.56,
      retrievedAt: "2026-04-14T04:00:51.071Z",
      supportMetadata: {
        product_area: "openapi",
        deployment_model: "shared",
    ...
    {
      documentId: "doc:private-deployment-admin-reset-low-score",
      evidenceId: "chunk:private-deployment-admin-reset-low-score",
      title: "Reset administrator password in private deployment",
      snippet:
        "When SMTP-based delivery is unavailable, use the private deployment recovery procedure to reset the administrator password directly in the deployment environment.",
      sourceUrl: "https://docs.ones.com/private-deployment/reset-administrator-password",
  ...
  try {
    const result = await coreRunSupportSearchAgent({
      ...
      adapter,
      orchestrator: orchestrator as never,
      idempotencyKey: "support-supervisor-domain-deployment-admin-reset-fallback"
    });
...
test("runSupportSearchAgent supervisor-domain runtime realigns deployment recovery procedures from troubleshooting planner output to how-to", async () => {
  const mutableEnv = env as {
    FEATURE_SUPPORT_AGENT_SINGLE_AGENT_RUNTIME?: boolean;
    OPENCLAW_AGENT_ID_SUPPORT_MAIN?: string;
  };
  ...
  globalThis.fetch = (async (input) => {
    ...
    throw new Error(`unexpected fetch in deployment recovery troubleshooting realignment test: ${requestUrl}`);
  }) as typeof fetch;
  ...
  adapter.planSupportDispatch = async () => ({
    ...
    route: {
      question_type: "troubleshooting",
      user_goal: "Reset the administrator password in a private deployment when the normal OAuth token delivery path is unavailable.",
      answer_contract:
        "Provide the documented private-deployment/admin-recovery procedure for resetting an administrator password without relying on OAuth token delivery, including any host-side prerequisites, recovery steps, and constraints.",
      specialist_agent: "troubleshooting-specialist",
      routing_confidence: 0.96,
      primary_domain: "deployment"
    },
    caseFrame: {
      goal: "Restore administrator access by resetting the admin password in a private/self-hosted deployment when OAuth token delivery is unavailable.",
      symptom: "Administrator password reset cannot use OAuth token delivery; admin is locked out or cannot complete the normal reset flow.",
      object: "administrator password reset for private deployment",
      action_type: "how_to",
      deployment_model: "private deployment",
      product_area: "deployment",
      constraints: [
        "Must assume private/self-hosted deployment context.",
        "Normal OAuth token delivery is unavailable, so SaaS-style email/token reset flow may not work."
      ],
      missing_critical_info: [],
      retrieval_queries: ["private deployment administrator password reset when OAuth token delivery unavailable"],
      question_type: "troubleshooting",
      specialist_agent: "troubleshooting-specialist",
      answer_contract:
        "Provide the documented private-deployment/admin-recovery procedure for resetting an administrator password without relying on OAuth token delivery, including any host-side prerequisites, recovery steps, and constraints.",
      ...
      primary_domain: "deployment",
      query_plan: {
        concept_queries: ["private deployment admin recovery", "password reset without OAuth token delivery"],
        object_queries: ["administrator password reset private deployment"],
        behavior_queries: ["documented fallback for admin password reset in private deployment", "how_to"]
      },
      required_doc_kinds: ["deployment_runbook", "troubleshooting", "rules"]
    },
    retrievalQueries: ["private deployment administrator password reset when OAuth token delivery unavailable"]
  });
  adapter.writeDeploymentDomainAnswer = async () => {
    throw new Error("simulate deployment specialist fallback");
  };
  ...
  const references: SearchReference[] = [
    {
      ...
      sourceType: "github_kb",
      score: 1.45,
      retrievedAt: "2026-04-14T05:10:00.000Z",
      supportMetadata: {
        product_area: "deployment",
        deployment_model: "private_deployment",
        evidence_kind: "procedure",
  ...
  try {
    const result = await coreRunSupportSearchAgent({
      ...
      adapter,
      orchestrator: orchestrator as never,
      idempotencyKey: "support-supervisor-domain-deployment-recovery-troubleshooting-realignment"
    });

    assert.equal(
      ((result.result.internal_diagnostics as { route?: { primary_domain?: string; specialist_agent?: string; question_type?: string } } | undefined)
    ...
    assert.equal(
      ((result.result.internal_diagnostics as { route?: { question_type?: string } } | undefined)?.route?.question_type),
      "how_to_product"
    );
    assert.equal(result.result.support_answer?.render_variant, "how_to");
    assert.notEqual(result.result.state, "TICKET_HANDOFF_RECOMMENDED");
    assert.equal(result.result.citations.some((item) => item.id === "chunk:private-deployment-admin-reset"), true);
    assert.match(result.result.answer, /reset the administrator password|recovery procedure/i);
  } finally {
    mutableEnv.FEATURE_SUPPORT_AGENT_SINGLE_AGENT_RUNTIME = originalSingleAgentRuntime;
    mutableEnv.OPENCLAW_AGENT_ID_SUPPORT_MAIN = originalSupportMainAgentId;
