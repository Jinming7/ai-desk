  ...
  const clauses = [
    `(
      (
        EXISTS (
          SELECT 1
          FROM kb_serving_versions sv
          WHERE sv.repo_id = entry.repo_id
            AND sv.branch = entry.branch
        )
        AND entry.status = 'active'
        AND entry.build_version = (
          SELECT sv.active_build_version
          FROM kb_serving_versions sv
          WHERE sv.repo_id = entry.repo_id
            AND sv.branch = entry.branch
          LIMIT 1
    ...
    `(
      (
        EXISTS (
          SELECT 1
          FROM kb_serving_versions sv
          WHERE sv.repo_id = profile.repo_id
            AND sv.branch = profile.branch
        )
        AND profile.build_version = (
          SELECT sv.active_build_version
          FROM kb_serving_versions sv
          WHERE sv.repo_id = profile.repo_id
            AND sv.branch = profile.branch
       ...
       AND (
         (
           EXISTS (
             SELECT 1
             FROM kb_serving_versions sv
             WHERE sv.repo_id = target.repo_id
               AND sv.branch = target.branch
           )
           AND target.status = 'active'
           AND target.build_version = (
             SELECT sv.active_build_version
             FROM kb_serving_versions sv
             WHERE sv.repo_id = target.repo_id
               AND sv.branch = target.branch
  ...
  const result = await pool.query<{
    ...
    `WITH ranked AS (
      ...
      WHERE src.memory_id = ANY($1::uuid[])
        AND (
          (sv.active_build_version IS NOT NULL AND entry.build_version = sv.active_build_version AND chunk.build_version = sv.active_build_version AND doc.build_version = sv.active_build_version)
          OR (sv.active_build_version IS NULL AND entry.status = 'active' AND entry.is_latest = true)
        )
    )
    SELECT * FROM ranked WHERE rn <= $2`,
    [memoryIds, input.limitPerMemory]
...
export async function listActiveMemoryEntriesForScope(input: {
  ...
  const clauses = ["repo_id = $1", "branch = $2", "status = 'active'"];
