    ...
    <div className="mx-auto grid max-w-7xl gap-6 px-4 py-8 md:grid-cols-[300px_1fr] md:px-8">
      ...
      <section className="space-y-4">
        {messages.map((message) => (
          <article
            key={message.id}
            className={`rounded-mdplus border p-4 ${message.is_ai_generated ? "border-brand-100 bg-brand-50/70" : "border-slate-200 bg-white"}`}
          >
            <div className="mb-2 flex items-center justify-between text-sm text-slate-500">
              <div className="flex items-center gap-2">
                {message.is_ai_generated ? <Bot size={16} className="text-brand-500" /> : <UserRound size={16} />}
            ...
            {message.is_ai_generated && (
              <div className="mt-3 rounded-mdplus border border-brand-200 bg-white p-2 text-xs text-slate-600">
                This response was generated by AI. Was it helpful? <button className="text-brand-500">Yes</button> / <button className="text-brand-500">No</button>
              </div>
            )}
          </article>
        ))}
